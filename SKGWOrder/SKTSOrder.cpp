...
CSKTSOrder::CSKTSOrder(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKTSOrder::~CSKTSOrder()
{
}
...
/// 送出委託
/// </summary>
void CSKTSOrder::Run(const TSKTSOrder& tSKOrder, const struct timeval& tvStart, const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKTSOrder Run " + g_strEMFatal;
        SetReplyData();
...
/// 確認User委託資料
/// </summary>
bool CSKTSOrder::SetPostData(const TSKTSOrder& tSKOrder)
{
    try
    {
        ...
        sprintf(caBuf, "{\"LoginID\":\"%s\",\"IDNO\":\"%s\",\"BrokerID\":\"%s\",\"ACNO\":\"%s\",\"StockID\":\"%s\""
            ...
            ",\"Price\":\"%s\",\"PriceMark\":\"%s\",\"ORKEY\":\"%s\",\"Source\":\"%s\",\"TranType\":\"%s\""
            ",\"RawData\":\"%s\",\"Sign\":\"%s\",\"CN\":\"%s\",\"CertID\":\"%s\",\"CertType\":\"%s\""
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKTSOrder SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKTSOrder SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKTSOrder::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        {
            "Code":"000",
                "Message" : "9182-9827666 預約單委託成功,交易日 1080604",
                "datas" : {
                "OrderNo":"99999",
                    "SeqNo" : "2310000000340",
                    "TDate" : "1080604",
                    "Status" : "9182-9827666 預約單委託成功,交易日 1080604",
                    "StockNo" : "1101",
                    "StockName" : "台 泥"
                    "Action" : "",
                    "BlockNum" : ""
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(TSOrder)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(TSOrder) = " + strData, false);
        ...
        if (iter1st != __Node->GetObj()->end())
        {
            if (iter1st->second->GetVlaueType() == web::json::kObject)
            {
                ...
                // Action
                iter2nd = iter1st->second->GetObj()->find("Action");
                if (iter2nd != iter1st->second->GetObj()->end())
                {
                    if (iter2nd->second->GetVlaueType() == web::json::kString)
                    {
                        strAction = *(iter2nd->second->GetString());
                        CUtility::TrimByRef(strAction);
                    }
                    else
                        strAction = "";
                }
                ...
                // BlockNum
                iter2nd = iter1st->second->GetObj()->find("BlockNum");
                if (iter2nd != iter1st->second->GetObj()->end())
                {
                    if (iter2nd->second->GetVlaueType() == web::json::kString)
                    {
                        strBlockNum = *(iter2nd->second->GetString());
                        CUtility::TrimByRef(strBlockNum);
                    }
                    else
                        strBlockNum = "";
                }
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKTSOrder Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKTSOrder Parse GW Data " + g_strEMFatal;
    }
...
/// 設定回傳資料
/// </summary>
void CSKTSOrder::SetReplyData()
{
    m_uncaReplyData[0] = ESCAPE_BYTE;
    m_uncaReplyData[1] = TSORDER_REP_BYTE;
    ...
    memcpy(m_uncaReplyData + 658, strAction.c_str(), strAction.size() > 1 ? 1 : strAction.size());
    memcpy(m_uncaReplyData + 659, strBlockNum.c_str(), strBlockNum.size() > 8 ? 8 : strBlockNum.size());

}

void CSKTSOrder::SetLogger()
{
    m_tSKLoggerGW.strFunction.assign("AddOrderLog");
    m_tSKLoggerGW.strUserID.assign(m_shpClient->GetLoginID());
    m_tSKLoggerGW.strFunc.assign("TSOrder");
...
/// </summary>
int CSKTSOrder::CheckOverTimeToReply(const TSKTSOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
