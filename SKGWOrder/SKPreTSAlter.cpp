#include "SKPreTSAlter.h"
#include "../json.h"
#include "../Utility.h"

CSKPreTSAlter::CSKPreTSAlter(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKPreTSAlter::~CSKPreTSAlter()
{
}
...
/// 送出委託
/// </summary>
void CSKPreTSAlter::Run(const TSKTSOrder& tSKOrder, const struct timeval& tvStart, const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKPreTSAlter Run " + g_strEMFatal;
        SetReplyData();
...
/// 確認User委託資料
/// </summary>
bool CSKPreTSAlter::SetPostData(const TSKTSOrder& tSKOrder)
{
    try
    {
      ...
      /*  if (strBrokerID.empty())
       ...
       /* if (strACNO.empty())
        ...
        string strNum = CUtility::TrimCA(tSKOrder.caNum, sizeof(tSKOrder.caNum));
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKPreTSAlter SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKPreTSAlter SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKPreTSAlter::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(PreTSAlter)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(PreTSAlter) = " + strData, false);
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKPreTSAlter Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKPreTSAlter Parse GW Data " + g_strEMFatal;
    }
...
/// 設定回傳資料
/// </summary>
void CSKPreTSAlter::SetReplyData()
{
...
/// </summary>
int CSKPreTSAlter::CheckOverTimeToReply(const TSKTSOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
