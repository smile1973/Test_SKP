...
CSKOFOrder::CSKOFOrder(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKOFOrder::~CSKOFOrder()
{
}
...
/// 送出委託
/// </summary>
void CSKOFOrder::Run(const TSKOFOrder& tSKOrder, const struct timeval& tvStart,const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
   ...
   /*     if (nLogModel == 0)
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKOFOrder Run " + g_strEMFatal;
        SetReplyData();
...
/// 確認User委託資料
/// </summary>
bool CSKOFOrder::SetPostData(const TSKOFOrder& tSKOrder)
{
    try
    {
        ...
        //下單必帶, 刪單可帶空
        string strOexchange = CUtility::TrimCA(tSKOrder.caOexchange, sizeof(tSKOrder.caOexchange));
        if (strMarketType != "" && strOexchange.empty())
            throw string("No Oexchange");

        //下單必帶, 刪單可帶空
        string strOpenKind = CUtility::TrimCA(tSKOrder.caOpenKind, sizeof(tSKOrder.caOpenKind));
        if (strMarketType != "" && strOpenKind.empty())
        ...
        if (strMarketType != "" && strCOND.empty())
        ...
        //非必帶
        string strTouchPrice = CUtility::TrimCA(tSKOrder.caTouchPrice, sizeof(tSKOrder.caTouchPrice));
        //if (strTouchPrice.empty()) 
        //    throw string("No TouchPrice");

        //非必帶
        string strTouchPrice_M = CUtility::TrimCA(tSKOrder.caTouchPrice_M, sizeof(tSKOrder.caTouchPrice_M));
        //if (strTouchPrice_M.empty()) 
        //    throw string("No TouchPrice_M");

        //非必帶
        string strTouchPrice_D = CUtility::TrimCA(tSKOrder.caTouchPrice_D, sizeof(tSKOrder.caTouchPrice_D));
        ...
        m_strPostJson += "\",\"OExchange\":\"";
        m_strPostJson += strOexchange;
        m_strPostJson += "\",\"OpenKind\":\"";
        m_strPostJson += strOpenKind;
        ...
        m_strPostJson += "\",\"TouchPrice\":\"";
        m_strPostJson += strTouchPrice;
        m_strPostJson += "\",\"TouchPrice_M\":\"";
        m_strPostJson += strTouchPrice_M;
        m_strPostJson += "\",\"TouchPrice_D\":\"";
        m_strPostJson += strTouchPrice_D;
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKOFOrder SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKOFOrder SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKOFOrder::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(OFOrder)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(OFOrder) = " + strData, false);
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKOFOrder Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKOFOrder Parse GW Data " + g_strEMFatal;
    }
...
/// </summary>
int CSKOFOrder::CheckOverTimeToReply(const TSKOFOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
