#include "SKPreOSOrder.h"
#include "../json.h"
#include "../Utility.h"

CSKPreOSOrder::CSKPreOSOrder(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKPreOSOrder::~CSKPreOSOrder()
{
}
...
/// 送出委託
/// </summary>
void CSKPreOSOrder::Run(const TSKOSOrder& tSKOrder, const struct timeval& tvStart, const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKPreOSOrder Run " + g_strEMFatal;
        SetReplyData();
        SetLogger();
        SendClientCallback();
    }
...
/// 確認User委託資料
/// </summary>
bool CSKPreOSOrder::SetPostData(const TSKOSOrder& tSKOrder)
{
    try
    {
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKPreOSOrder SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKPreOSOrder SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKPreOSOrder::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(PreOSOrder)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(PreOSOrder) = " + strData, false);
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKPreOSOrder Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKPreOSOrder Parse GW Data " + g_strEMFatal;
    }
...
/// 設定回傳資料
/// </summary>
void CSKPreOSOrder::SetReplyData()
{
...
/// </summary>
int CSKPreOSOrder::CheckOverTimeToReply(const TSKOSOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
