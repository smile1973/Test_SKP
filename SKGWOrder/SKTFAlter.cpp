...
CSKTFAlter::CSKTFAlter(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKTFAlter::~CSKTFAlter()
{
}
...
/// 送出委託
/// </summary>
void CSKTFAlter::Run(const TSKTFOrder& tSKOrder, const struct timeval& tvStart, const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKTFAlter Run " + g_strEMFatal;
        SetReplyData();
        SetLogger();
        SendClientCallback();
    }
...
/// 確認User委託資料
/// </summary>
bool CSKTFAlter::SetPostData(const TSKTFOrder& tSKOrder)
{
    try
    {
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKTFAlter SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKTFAlter SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKTFAlter::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(TFAlter)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(TFAlter) = " + strData, false);
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKTFAlter Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKTFAlter Parse GW Data " + g_strEMFatal;
    }
...
/// </summary>
int CSKTFAlter::CheckOverTimeToReply(const TSKTFOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
