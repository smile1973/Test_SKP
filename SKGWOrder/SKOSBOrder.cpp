#include "SKOSBOrder.h"
#include "../json.h"
#include "../Utility.h"

CSKOSBOrder::CSKOSBOrder(const string& strServer) : CSKBaseGWOrder(strServer)
{
    m_tSKLoggerGW = {};
}

CSKOSBOrder::~CSKOSBOrder()
{
}
...
/// 送出委託
/// </summary>
void CSKOSBOrder::Run(const TSKOSOrder& tSKOrder, const struct timeval& tvStart, const string& strThreadID)
{
    try
    {
        m_tvStart = tvStart;
    ...
    catch (...)
    {
        ...
        m_strErrMsg = "CSKOSBOrder Run " + g_strEMFatal;
        SetReplyData();
        SetLogger();
        SendClientCallback();
    }
...
/// 確認User委託資料
/// </summary>
bool CSKOSBOrder::SetPostData(const TSKOSOrder& tSKOrder)
{
    try
    {
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKOSBOrder SetPostData " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKOSBOrder SetPostData " + g_strEMFatal;
    }
...
/// 確認回傳資料
/// </summary>
void CSKOSBOrder::ParseGWData(const string& strData)
{
    ...
    try
    {
        ...
        */
        ...
        if (!parser.Parse(strData, &__Node))
        {
            ...
            {
                write_log_(e_ltServerError, "(OSBOrder)連不上srvgw或是proxy server呼叫的url有誤", false);
                throw g_strEMGWConnFail;
            }
            else
            {
                write_log_(e_ltServerError, "receive Data from srvgw(OSBOrder) = " + strData, false);
    ...
    catch (string strMsg)
    {
        ...
        m_strErrMsg = "CSKOSBOrder Parse GW Data " + strMsg;
    }
    catch (...)
    {
        m_strErrCode = g_strECFatal;
        m_strErrMsg = "CSKOSBOrder Parse GW Data " + g_strEMFatal;
    }
...
/// 設定回傳資料
/// </summary>
void CSKOSBOrder::SetReplyData()
{
    m_uncaReplyData[0] = ESCAPE_BYTE;
    m_uncaReplyData[1] = OSBORDER_REP_BYTE;
...
/// </summary>
int CSKOSBOrder::CheckOverTimeToReply(const TSKOSOrder& tSKOrder, const struct timeval& tvStart)
{
    try
    {
        m_tvStart = tvStart;
